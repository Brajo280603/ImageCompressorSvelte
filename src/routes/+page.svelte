    ...
    // Implement your image compression logic here
		...
		while (currentProcessingIndex < allFiles.length) {
			...
			for (let i = 0; i < 10 && currentProcessingIndex < allFiles.length; i++) {
				let Fileblob = await imageCompressorFunction(allFiles[currentProcessingIndex]);
  ...
  <div class="h-full w-full grid grid-cols-1 gap-5 justify-items-center">
    ...
    <div class="space-y-2">
      <label class="flex items-center space-x-2">
        ...
        <input type="number" placeholder="FileSize" class="rounded-full disabled:variant-filled-surface px-3 py-1 w-24 text-slate-900" disabled="{optionSelect !== 'Size'}" bind:value={imageSize}> 
        <span>KB</span>
      </label>
      <!-- <label class="flex items-center space-x-2">
    ...
    <div>
      <button type="button" class="btn variant-glass-primary" on:click={() => compress_image()}>Compress</button>
    </div>
  </div>

	<div class="ps-5">
		<p>number of images processed : {currentProcessingIndex}</p>
		<ProgressRadial value={((currentProcessingIndex/files.length)*100).toFixed(2)} stroke={100} meter="stroke-primary-500" track="stroke-primary-500/30" strokeLinecap="round">{((currentProcessingIndex/files.length)*100).toFixed(2)}%</ProgressRadial>
	</div>

	<div class="images flex flex-col w-50 items-center mt-12">
